...
function LogActivity($UserId,$Action,$Ticket=null,$Activity,$Datetime) {
	global $DBSMART;
    
	$dblog = new clsDBSMART() ;
	$sqllog = "INSERT INTO smart_logactivity (id,log_userid,log_action,log_ticket,log_description,log_date) VALUES ('','".$UserId."','".$Action."','".$Ticket."','".$Activity."','".$Datetime."')";
	//print("SQL=>".$sqllog);
	//die();
	$dblog->query($sqllog) ;
}
...
function BroadcastEmail($typenotify,$to,$from=null,$title,$content=null,$dateemel=null) {
	...
	switch($typenotify) {
		...
		case 'newpm' :
			...
			$subject = 'SMART '.$title.': '.$content.' - '.$EngineerUsername;
			$message = "<html><head><title>SMART</title></head>";
			$message .= "<body>";
			$message .= "<h3>Sallam & Good Day!</h3>";
			$message .= "<br>This is an automatic notification from SMART System</br>";
			$message .= "<p>Please be informed that a new PM Report <b>(#:".$content.")</b> has been created on <b>".date("d/m/Y h:m:s",strtotime($dateemel))."</b> by : <b>".$Engineer."</b></p>";
			$message .= "<p>Kindly login to SMART via  <a href='http://smart.target.net.my/SMART/index.php' target='_blank'>http://smart.target.net.my/SMART/index.php</a> 
							...
							<b><em>http://smart.target.net.my/SMART/pmactivity.php?pmid=".$PmID."&rf=".$content."</em></b> </p>";
			$message .= "<hr><br>";
			$message .= "<em>Please do not reply to this email. Should you have any enquiry, please contact us via software@target.net.my</em>";
			$message .= "<br>Thank you</br>";
			$message .= "</body></html>";
		...
		case 'newcm' :
			...
			$message .= "OPENED : ".date("D d/m/y H:i",strtotime($r_date))."<br>";
			$message .= "CLOSED : ".date("D d/m/y H:i",strtotime($c_date))."<br>";
