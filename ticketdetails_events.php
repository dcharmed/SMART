...
// -------------------------
	...
	if(CCGetFromGet("id")==null) {
		$tempIdForRefNumber = GetAutoGenerateTicket(date('Y'),date('m'));
		...
		$idTask = CCDLookUp("max(id)","smart_task","ticket_id=".CCGetParam("id"), $DBSMART);
		...
		$smart_ticket->tckt_engineer->SetValue(strtoupper(CCDLookUp("usr_username","smart_user","id=".$latestEng,$DBSMART)));
	}
	
	$smart_ticket->tckt_method->SetValue(strtoupper(CCDLookUp("ref_description","smart_referencecode","ref_value=".$smart_ticket->tckt_method->GetValue()." AND ref_type='rsltnmethod'",$DBSMART)));
...
// -------------------------
	...
	$monthTicket = substr($smart_ticket->tckt_refnumber->GetValue(), 2, 4);
	...
	LogStatusChange(CCGetSession("UserLogin"),1,$refnumber,date('Y-m-d H:i:s'));
	UpdateAutoGenerateTicket("20".$yearTicket,$monthTicket);
...
// -------------------------
	...
	$BilRekod = $BilRekod + 1;
	if($GResNote->rsltn_type->GetValue() == "SN") {
		$GResNote->rsltn_date->SetLink("ticketdetails.php?id=".CCGetFromGet("id")."&rid=".$GResNote->rsltn_id->GetValue());
		$GResNote->rsltn_spart->SetValue("N/A");
		$GResNote->rsltn_eq->SetValue("N/A");
		$GResNote->rsltn_att->SetValue("N/A");
	} elseif($GResNote->rsltn_type->GetValue() == "CM") {
		$GResNote->rsltn_date->SetLink("cmactivity.php?tcktid=".CCGetFromGet("id")."&rid=".$GResNote->rsltn_id->GetValue());
		
		//CHECKING CM DETAILS
		$sqlRplc = "SELECT smart_resolutionnote.ticket_id, SUM(IF(rplc_type='sp',1,0)) AS countedspart,SUM(IF(rplc_type='eq',1,0)) AS  countedeq
					   from smart_replacement INNER JOIN smart_resolutionnote 
					   ON smart_resolutionnote.id=smart_replacement.resolution_id 
					   AND smart_resolutionnote.ticket_id=".CCGetFromGet("id"). " GROUP BY smart_replacement.resolution_id";
		$db->query($sqlRplc);
		$ResultRplc = $db->next_record();
		if ($ResultRplc) {
			$spartdet = $db->f("countedspart");
			$eqdet = $db->f("countedeq");
		}
		$sqlAtt = "SELECT COUNT(smart_attachment.attch_name) AS countedatt from smart_attachment 
					WHERE resolution_id=".$GResNote->rsltn_id->GetValue()." GROUP BY smart_attachment.resolution_id";
		//echo "<br>";
		$db->query($sqlAtt);
		$ResultAtt = $db->next_record();
		if ($ResultAtt) {
			$attdet = $db->f("countedatt");
		}

		if($spartdet > 0) {
			$GResNote->rsltn_spart->SetValue("YES");
		} else {
			$GResNote->rsltn_spart->SetValue("N/A");
		}
	
		if($eqdet > 0) {
			$GResNote->rsltn_eq->SetValue("YES");
		} else {
			$GResNote->rsltn_eq->SetValue("N/A");
		}

		if($attdet > 0) {
			$GResNote->rsltn_att->SetValue("YES");
		} else {
			$GResNote->rsltn_att->SetValue("N/A");
		}
	}
...
function GResNote_BeforeShow(& $sender)
{
    ...
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $GResNote, $DBSMART,$BilRekod, $PageFirstRecordNo; //Compatibility
...
// -------------------------
	...
	if($currstatus == 5) $smart_task->task_newstatus->DataSource->Where = " ref_type='tcktstatus' AND ref_value >=3";
    else $smart_task->task_newstatus->DataSource->Where = " ref_type='tcktstatus' AND ref_value >=".$currstatus;
// -------------------------
//End Custom Code
...
// -------------------------
    $currstatus = CCDLookUp("tckt_status","smart_ticket","id=".CCGetParam("id"),$DBSMART);
	$curreng = CCDLookUp("max(task_currenteng)","smart_task","ticket_id=".CCGetParam("id")." AND task_status<1",$DBSMART);
...
// -------------------------
	...
	$datetask = $smart_task->task_date->GetValue();
	$notes = $smart_task->Reason->GetValue();

	$dbupd = new clsDBSMART() ;
	if($task_newstatus == 2 || $task_newstatus == 3) {
	    $sqlupd = "INSERT INTO smart_task(ticket_id, task_update, task_currenteng, task_updatedeng, task_current, task_date, task_notes, datemodified)";
		$sqlupd .= " VALUES ('".$ticket_id."', '".$task_newstatus."', '".$task_pic."', '".$task_spic."', '".$task_oldstatus."', '".$datetask."','".$notes."', '".$datemodified."')";
		$dbupd->query($sqlupd);
		if($task_spic!=null) {
			$sqlUpdEng = "UPDATE smart_ticket SET tckt_engineer='".CCDLookUp("usr_username","smart_user","id=".$task_spic,$DBSMART)."' WHERE id = ".$ticket_id;
			$dbupd->query($sqlUpdEng);
		} else {
			$sqlUpdEng = "UPDATE smart_ticket SET tckt_engineer='".CCDLookUp("usr_username","smart_user","id=".$task_pic,$DBSMART)."' WHERE id = ".$ticket_id;
			$dbupd->query($sqlUpdEng);
		}
	} elseif($task_oldstatus == 1 && $task_newstatus == 7) {
		$sqlupd = "INSERT INTO smart_task(ticket_id, task_status, task_update, task_currenteng, task_updatedeng, task_current, task_date,task_notes, datemodified)";
		$sqlupd .= " VALUES ('".$ticket_id."', '1', '".$task_newstatus."', '".CCGetSession("UserID")."', '', '".$task_oldstatus."', '".$datetask."','".$notes."', '".$datemodified."')";
		$dbupd->query($sqlupd);
	}
	...
	LogStatusChange(CCGetSession("UserLogin"),$task_newstatus,GetCodeFromSingleTable('smart_ticket',$ticket_id,'tckt_refnumber'),$datemodified);
// -------------------------
//End Custom Code
