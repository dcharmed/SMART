<?php
//Include Common Files @1-88DB1508
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "ticketlist.php");
...
//Class_Initialize Event @6-D2AE0A92
    function clsRecordsmart_ticketSearch($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->s_status = & new clsControl(ccsListBox, "s_status", "s_status", ccsText, "", CCGetRequestParam("s_status", $Method, NULL), $this);
            $this->s_status->DSType = dsTable;
            $this->s_status->DataSource = new clsDBSMART();
            $this->s_status->ds = & $this->s_status->DataSource;
            $this->s_status->DataSource->SQL = "SELECT * \n" .
"FROM smart_referencecode {SQL_Where} {SQL_OrderBy}";
            list($this->s_status->BoundColumn, $this->s_status->TextColumn, $this->s_status->DBFormat) = array("ref_value", "ref_description", "");
            $this->s_status->DataSource->Parameters["expr72"] = tcktstatus;
            $this->s_status->DataSource->wp = new clsSQLParameters();
            $this->s_status->DataSource->wp->AddParameter("1", "expr72", ccsText, "", "", $this->s_status->DataSource->Parameters["expr72"], "", false);
            $this->s_status->DataSource->wp->Criterion[1] = $this->s_status->DataSource->wp->Operation(opEqual, "ref_type", $this->s_status->DataSource->wp->GetDBValue("1"), $this->s_status->DataSource->ToSQL($this->s_status->DataSource->wp->GetDBValue("1"), ccsText),false);
            $this->s_status->DataSource->Where = 
                 $this->s_status->DataSource->wp->Criterion[1];
...
"FROM smart_referencecode {SQL_Where} {SQL_OrderBy}";
            ...
            $this->s_state->DataSource->Parameters["expr79"] = state;
            $this->s_state->DataSource->wp = new clsSQLParameters();
            $this->s_state->DataSource->wp->AddParameter("1", "expr79", ccsText, "", "", $this->s_state->DataSource->Parameters["expr79"], "", false);
...
//Validate Method @6-5225CF23
    function Validate()
    {
        ...
        $Validation = ($this->s_status->Validate() && $Validation);
        ...
        $Validation =  $Validation && ($this->s_status->Errors->Count() == 0);
...
//CheckErrors Method @6-D9E0BF05
    function CheckErrors()
    {
        ...
        $errors = ($errors || $this->s_status->Errors->Count());
    ...
    function Operation()
    {
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "ticketlist.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y", "Button_Reset", "Button_Reset_x", "Button_Reset_y")));
...
//Show Method @6-D30C4886
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            ...
            $Error = ComposeStrings($Error, $this->s_status->Errors->ToString());
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "ticketlist.html";
$BlockToParse = "main";
...
//Include events file @1-15FB378E
include_once("./ticketlist_events.php");
